...
We answer this with a unified generative-predictive model that represents RGB, depth, and camera rays as spatially aligned tokens in a shared multimodal transformer and trains it via *diffusion forcing* <d-cite key="chen2024diffusionforcing"></d-cite>. The clean/noisy pattern across modality-frame groups specifies the task. The same weights then perform camera-controlled video generation, dense video depth estimation, and camera pose recovery --- and crucially, the generative formulation lets us *sample* multiple 4D scene hypotheses at inference time and pick the one with the lowest cross-modal reprojection error.

<div class="l-page">
  <figure>
    <img src="{{ '/assets/img/4d-world-model/teaser2.png' | relative_url }}" alt="One model, three modalities, many tasks."/>
    <figcaption>
      <b>Figure 1 &middot; Unified conditional completion.</b> Given any partial observation --- a text prompt, a single RGB image, an RGB sequence, camera poses (rays), or depth --- the same model fills in the rest. Camera-controlled generation, depth estimation, and pose recovery are different masks on the same denoiser.
    </figcaption>
  </figure>
</div>
...
To put camera geometry in the same spatial container as RGB and depth we convert $C_i$ into a **dense Plücker ray map**:

$$R_i(u,v) = \big(\,\mathbf{d}_i(u,v),\ \mathbf{m}_i(u,v)\,\big), \qquad \mathbf{m}_i(u,v) = \mathbf{o}_i \times \mathbf{d}_i(u,v),$$

where $\mathbf{o}_i$ is the camera center and $\mathbf{d}_i(u,v)$ the unit viewing direction in world coordinates. Ray direction carries orientation + intrinsics; ray moment carries the camera center up to a global scene scale. Depth is reparameterized to a bounded disparity-like channel
...
This is where the formulation pays off. Each training sample carries a modality-availability mask; missing streams are filled with Gaussian placeholders, forced to max noise, and zeroed out of the loss. So an RGB-only YouTube clip and a fully annotated synthetic clip train the *same* model side by side --- the loss simply lights up different terms.

Training mixture (re-weighted by dataset size): RealEstate10K (RGB+pose, 0.5), DL3DV-10K (RGB+pose+depth via VIPE, 5.0), SpatialVID (RGB+pose+text, 1.0), OmniWorld (1.5), ScanNet++ (30.0), Virtual KITTI 2 (50.0), PointOdyssey (50.0), AgiBot World (RGB only, 0.04). Optimization: AdamW, lr $1\!\times\!10^{-5}$, betas $(0.9,0.95)$, weight decay 0.05, grad clip 5.0, 1k warmup, constant schedule. 4&times; H200 GPUs, effective batch 16 via grad-accum 4, $\sim$30k steps, bf16 mixed precision, gradient checkpointing.
...
with $\tilde{I}_{i\to j}^{(\ell)}$ the warp from frame $i$ to frame $j$, $M_{ij}^{(\ell)}$ the valid-pixel mask, and $\mathcal{P}$ an exhaustive pair set over a small anchor subsample. We pick $\ell^\star = \arg\min_\ell e^{(\ell)}$. Candidates whose RGB, depth, and rays disagree under their own geometry warp badly and self-eliminate. $L=16$ throughout.
...
**Baselines.** Wan2.1-FLF-14B (no camera input) <d-cite key="wan2025wan"></d-cite>; DFoT <d-cite key="song2025dfot"></d-cite>, a diffusion-forcing video completer; SEVA <d-cite key="zhou2025seva"></d-cite>, a state-of-the-art NVS-specialist diffusion model.

<div class="l-page">
  <figure>
    <div class="vid-grid" style="grid-template-columns: repeat(4, 1fr);">
      <div><video autoplay loop muted playsinline src="{{ '/assets/video/4d-world-model/nvs_gt.mp4' | relative_url }}"></video><div style="text-align:center; font-size:0.8em;">Ground truth</div></div>
      <div><video autoplay loop muted playsinline src="{{ '/assets/video/4d-world-model/nvs_ours.mp4' | relative_url }}"></video><div style="text-align:center; font-size:0.8em;"><b>Ours</b></div></div>
      <div><video autoplay loop muted playsinline src="{{ '/assets/video/4d-world-model/nvs_seva.mp4' | relative_url }}"></video><div style="text-align:center; font-size:0.8em;">SEVA</div></div>
      <div><video autoplay loop muted playsinline src="{{ '/assets/video/4d-world-model/nvs_wan.mp4' | relative_url }}"></video><div style="text-align:center; font-size:0.8em;">Wan2.1-FLF</div></div>
    </div>
    <figcaption><b>Figure 4 &middot; Camera-controlled generation.</b> SpatialVID samples. Ours preserves dynamic content along the requested trajectory; SEVA is biased toward static scenes; Wan2.1-FLF has no camera input and drifts on complex motion.</figcaption>
  </figure>
</div>

**Table 1 &middot; Camera-controlled generation.** PSNR&uarr; / LPIPS&darr; / SSIM&uarr;. Best in **bold blue**, second in light blue.
...
<div class="l-body">
  <figure>
    <div style="display:flex; gap:6px; align-items:flex-end;">
      <div style="flex:2"><img src="{{ '/assets/img/4d-world-model/pose_input.png' | relative_url }}" alt="input"/><div style="text-align:center;font-size:0.8em;">RGB frames</div></div>
      <div style="flex:1"><img src="{{ '/assets/img/4d-world-model/pose_gt.png' | relative_url }}" alt="gt"/><div style="text-align:center;font-size:0.8em;">GT trajectory</div></div>
      <div style="flex:1"><img src="{{ '/assets/img/4d-world-model/pose_ours.png' | relative_url }}" alt="ours"/><div style="text-align:center;font-size:0.8em;">Ours</div></div>
    </div>
    <figcaption><b>Figure 5 &middot; Predicted camera trajectory</b> on DL3DV-10K. Frustums show recovered poses; ours tracks the ground-truth motion closely.</figcaption>
  </figure>
</div>

**Table 2 &middot; Pose estimation.** ATE&darr; / RPE$_t$&darr; / RPE$_r$&darr; ($\times\!10^{-2}$).
...
<div class="l-page">
  <figure>
    <div style="display:grid; grid-template-columns: repeat(5, 1fr); gap: 4px;">
      ...
      <div><img src="{{ '/assets/img/4d-world-model/depth_gt.png' | relative_url }}" alt="GT"/></div>
      ...
      <div><img src="{{ '/assets/img/4d-world-model/depth_chrono.png' | relative_url }}" alt="Chrono"/></div>
    </div>
    <figcaption style="text-align:center;"><b>Figure 6 &middot; Video depth qualitative.</b> RGB &middot; GT &middot; <b>ours</b> &middot; Geo4D &middot; ChronoDepth on DL3DV-10K. Ours preserves sharper object boundaries.</figcaption>
  </figure>
</div>

**Table 3 &middot; Depth prediction.** AbsRel&darr; / RMSE&darr; / $\delta\!<\!1.25$&uarr;.
...
The TTS rows in Tables 2 and 3 are the same model, with $L=16$ samples and reprojection-based selection. TTS improves on the no-search baseline in 9/12 pose cells (dense) and 11/12 depth cells (sparse). Gains are not uniform: reprojection error is a *proxy* for geometric accuracy, so on textureless or specular regions the selector is misled. Still --- improvements at zero training cost, monotone in $L$ on DL3DV (Fig. 7), and consistent with the cognitive view of perception as generate-and-check <d-cite key="kersten2004object,yuille2006vision,battaglia2013simulation"></d-cite>.
...
<details>
<summary><b>Implementation notes</b> (architecture, schedule, training mixture)</summary>
<p>
<b>Base &middot;</b> Wan2.1-T2V-1.3B; 30 layers, hidden 1536, FFN 8960, 12 heads; VAE stride $[4,8,8]$, patch $[1,2,2]$; ray latents bypass the VAE and pack four consecutive 3-channel ray frames into a 16-channel latent. New modality patch embeddings and output heads are initialized from the pretrained RGB branch. We use 3D RoPE over latent $(u,v,i)$.<br/>
<b>Training mix (relative weights) &middot;</b> RealEstate10K (0.5), DL3DV-10K (5.0), SpatialVID (1.0), OmniWorld (1.5), ScanNet++ (30.0), Virtual KITTI 2 (50.0), PointOdyssey (50.0), AgiBot World (0.04). Modes 1--5 in Sec.&nbsp;3.3 are sampled per batch, restricted to a dataset's available modalities. Loss is the equally-weighted mean of $\mathcal{L}_I + \mathcal{L}_{R_d} + \mathcal{L}_{R_m} + \mathcal{L}_D$ with availability masking.<br/>
<b>Optimization &middot;</b> AdamW lr $1\!\times\!10^{-5}$, $(\beta_1,\beta_2)=(0.9,0.95)$, wd $0.05$, grad clip $5.0$, 1k warmup, constant schedule; bf16 mixed precision; 4&times; H200, effective batch $16$ via grad-accum $4$, $\sim$30k steps. History tokens are clean with prob. $0.5$, otherwise randomly low-noised.<br/>
<b>Inference &middot;</b> UniPC 40 steps, $w_{\mathrm{hist}}=2.0$. One 50-frame clip $\approx$ 1 min on a single H100. TTS uses $L=16$ candidates and an anchor-pair photometric reprojection scorer.
</p>
</details>
